...
%
%    b) use just two splittings instead of the three proposed in the paper.
%       
% The version clsunsal_v1 implements exactly the splitting scheme shwon in
% the paper
%
% ------------------------------------------------------------------------
%%  ===== Required inputs =============
...
% Modification: 
%
%     Jose Bioucas-Dias, July 2014 - correction of the projection onto R_+
%
%     
...
% lambda must be either a scalar or a p-vector
if numel(lambda) == 1
    lambda = lambda*ones(p,1);
elseif numel(lambda) == p
     lambda = lambda(:);
else
   error('inconsistent lambda size');
end
